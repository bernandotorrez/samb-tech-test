...
class BaseService implements BaseInterface
{
    ...
    /**
     * Get All Data Active and Show (Status = 1, is_show = 1)
     * @return Collection
     */
    public function allActiveAndShow()
    {
        return $this->model->select($this->searchableColumn)->where('status', '1')->where('is_show', '1')
     ...
     * @return Collection
     */
    public function allActiveAndShowRelation(array $with)
    {
        return $this->model->select($this->searchableColumn)->where('status', '1')->where('is_show', '1')
